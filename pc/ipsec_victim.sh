...
ip xfrm state
ip xfrm policy add src $vic_ip dst $serv_ip proto ip sport $vic_port dport $serv_port dir out ptype main tmpl src $vic_ip dst $serv_ip proto esp reqid 1 mode transport
ip xfrm policy add src $serv_ip dst $vic_ip proto ip sport $serv_port dport $vic_port dir in ptype main tmpl src $serv_ip dst $vic_ip proto esp reqid 2 mode transport
ip xfrm policy ls
